 ...
 * concession response to viewport change, and files/details staying mounted
...
import { FILES_DEFAULT, SIDEBAR_COLLAPSED } from '@deepseek-ai/dsh-client-ui-layout/src/client/columns.ts'
...
/** Track widths in grid order: sidebar, files, details (center is 1fr). */
...
describe('AppFrame', () => {
  ...
  it('renders the session pair with empty owner shares (sessionId is framework-standard)', () => {
    ...
    expect(slotCalls.find(c => c.key === 'files')!.props).toEqual({})
  ...
  it('renders all column occupants before baselines settle (no loading gate)', () => {
    ...
    expect(slotCalls.map(c => c.key)).toContain('conversation')
    expect(slotCalls.map(c => c.key)).toContain('files')
    expect(slotCalls.map(c => c.key)).toContain('details')
  })

  it('opens and closes the files panel independently of the session selection', () => {
    const { frame, instance } = mountFrame()
    expect(tracks(frame)).toEqual([280, 0, 0])

    act(() => { instance.actions.openFiles() })
    expect(tracks(frame)).toEqual([280, FILES_DEFAULT, 0])

    // Files stays open while the details panel toggles around it.
    act(() => { instance.actions.openDetails() })
    expect(tracks(frame)).toEqual([280, FILES_DEFAULT, 360])
  ...
  it('files drag widens rightward (positive dx grows the panel)', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openFiles() })
    const handles = frame.querySelectorAll('[class*="handle"]')
    // With only files open the handles are [sidebar, files]; the files
    // handle sits at the column's right edge (sidebar + files = 700).
    drag(handles[1]!, 700, 760)
  ...
  it('details drag widens leftward (negative dx grows the panel)', () => {
    ...
    act(() => { instance.actions.openDetails() })
    const handles = frame.querySelectorAll('[class*="handle"]')
    // With only details open the handles are [sidebar, details].
  ...
  it('files and details columns stay mounted at zero width', () => {
    const { frame, getByTestId } = mountFrame()
    expect(tracks(frame)).toEqual([280, 0, 0])
    expect(getByTestId('files-content')).toBeTruthy()
    expect(getByTestId('details-content')).toBeTruthy()
    expect(frame.hasAttribute('data-files-collapsed')).toBe(true)
  ...
  it('viewport shrink triggers the concession chain via ResizeObserver', () => {
    const { frame, instance } = mountFrame()
    act(() => { instance.actions.openFiles(); instance.actions.openDetails() })
    expect(tracks(frame)).toEqual([280, 420, 360])
    frameWidth = 1700 // still fits all three preferred widths
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
    expect(tracks(frame)).toEqual([280, 420, 360])
    frameWidth = 1650 // details concedes inside its range
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
    expect(tracks(frame)).toEqual([280, 420, 310])
    frameWidth = 1600 // details cannot fit at min → auto-closes
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
    expect(tracks(frame)).toEqual([280, 420, 0])
    frameWidth = 1250 // files concedes to 330
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
    expect(tracks(frame)).toEqual([280, 330, 0])
    frameWidth = 1100 // files closes (stays above the sidebar auto-collapse breakpoint); center absorbs
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
    expect(tracks(frame)).toEqual([280, 0, 0])
    frameWidth = 1920
    act(() => { fireResize?.(); vi.advanceTimersByTime(20) })
