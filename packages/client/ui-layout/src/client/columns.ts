 ...
 * then the same for the files panel. The sidebar never concedes: its
 ...
 * and files resolve to zero width.
...
export interface Columns { sidebar: number; center: number; files: number; details: number }
...
export const FILES_MIN = 320
/** Files drag clamp ceiling. */
export const FILES_MAX = 720
/** Files width before any user drag. */
export const FILES_DEFAULT = 420
 ...
 * @param files - files panel width preference in px (0 = closed).
 ...
 */
export function computeColumns(viewport: number, sidebar: number, files: number, details: number): Columns {
  ...
  if (s + f0 + d0 + CENTER_MIN <= viewport) {
    return { sidebar: s, center: viewport - s - f0 - d0, files: f0, details: d0 }
  }

  // Step 2: shrink details toward its minimum (the rightmost, most auxiliary
  // panel concedes before the files panel).
  const d1 = d0 === 0 ? 0 : Math.max(DETAILS_MIN, viewport - s - f0 - CENTER_MIN)
  if (s + f0 + d1 + CENTER_MIN <= viewport) return { sidebar: s, center: CENTER_MIN, files: f0, details: d1 }

  // Step 3: auto-close details (derived — preferences untouched).
  if (s + f0 + CENTER_MIN <= viewport) return { sidebar: s, center: viewport - s - f0, files: f0, details: 0 }

  // Step 4: shrink files toward its minimum.
  const f1 = f0 === 0 ? 0 : Math.max(FILES_MIN, viewport - s - CENTER_MIN)
  if (s + f1 + CENTER_MIN <= viewport) return { sidebar: s, center: CENTER_MIN, files: f1, details: 0 }

  // Step 5: auto-close files (derived); center absorbs any remaining deficit
  // (may drop below CENTER_MIN).
  return { sidebar: s, center: Math.max(0, viewport - s), files: 0, details: 0 }
}
