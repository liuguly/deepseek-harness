...
| `ctx.knowledge` | `core` | `knowledge` | - | - | - | 运行两条摄取流水线、搜索编排、变式生成与 /api/knowledge.* Fetch 路由；浏览器工作台是其唯一消费方。 |
| `ctx.knowledgeStore` | `seam` | `knowledge-pgvector` | `knowledge-pgvector` | `knowledge` | - | 幂等地引导数据库、扩展与 schema，然后提供文档、分块和模板产物的持久化，以及带结构化过滤的余弦 KNN。 |
| `ctx.knowledgeEmbedding` | `seam` | `knowledge-embedding-openai` | `knowledge-embedding-openai` | `knowledge` | - | 向配置的端点发送批量 /embeddings 请求，并对每个向量强制校验配置的维度。 |
